...
class LaTeXCompiler:
    ...
    def compile_latex(self, input_file: str) -> None:
        ...
        try:
            subprocess.run(
                ["latexmk", "-pdf", "-lualatex", "-synctex=1", input_file],
                check=True,
    ...
    def check_compilation_needs(self, input_file: str) -> None:
        ...
        try:
            ...
            if not os.path.exists(bbl_file):
                ...
                subprocess.run(
                    ["biber", os.path.splitext(input_file)[0]],
                    check=False,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                )
            ...
            glossary_files = [
                os.path.splitext(input_file)[0] + ext for ext in [".gls", ".acn"]
            ]
            for glossary_file in glossary_files:
                if not os.path.exists(glossary_file):
                    logging.info("Running makeglossaries...")
                    subprocess.run(
                        ["makeglossaries", os.path.splitext(input_file)[0]],
                        check=False,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.STDOUT,
                    )
            ...
            if not os.path.exists(idx_file):
                ...
                subprocess.run(
                    ["makeindex", os.path.splitext(input_file)[0]],
                    check=False,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                )
...
class LaTeXCompilerExecutor:
    ...
    def compile_with_threads(
        self, input_file: str, clean_previous: bool = False
    ) -> None:
        """
...
def main() -> None:
    ...
    try:
        ...
        clean_previous: bool = (
            input("Clean previous compilation files? (y/n): ").lower() == "y"
        )
